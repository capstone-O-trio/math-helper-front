...
export const useAppletakeoutTemplate = ({
  ...
  function onResults(results: any) {
    ...
    objectsRef.current.forEach(({ x, y, isObj }) => {
      ...
      const oy = y;
      if (
        isInDropZone(ratio, tree_dx, tree_dy, tree_dw, tree_dh, ox, oy, isObj)
      ) {
        // 객체가 드롭존 안에 있다면
        newTreeTotalNum++; // 총 개수 하나 증가
      } else if (
        isInDropZone(ratio, box_dx, box_dy, box_dw, box_dh, ox, oy, isObj)
      ) {
        // 객체가 드롭존 안에 있다면
        newBoxTotalNum++; // 총 개수 하나 증가
      }
    });
    if (newTreeTotalNum !== treeTotalNumRef.current)
      setTreeTotalNum(newTreeTotalNum);
    else if (newBoxTotalNum !== boxTotalNumRef.current)
      setBoxTotalNum(newBoxTotalNum);
