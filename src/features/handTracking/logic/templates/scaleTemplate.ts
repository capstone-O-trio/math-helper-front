...
export const useScaleTemplate = ({
  ...
  function onResults(results: any) {
    ...
    objectsRef.current.forEach(({ id, x, y, isObj }) => {
      const ox = x;
      const oy = y;
      if (
...
/* 1600 x 900을 기준으로 배치 */
function getScaleTemplateObjects(entityList: probEntityType[]): {
  objectsInfo: Obj[];
  objWeightInfo: objWeight[];
} {
